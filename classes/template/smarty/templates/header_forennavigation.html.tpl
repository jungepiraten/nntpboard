{foreach from=$curboard.childs item=board_item name=counter}
	<li class="{if isset($board) && $board_item.boardid == $board.boardid}active{/if}">
		<a href="viewboard.php?boardid={$board_item.boardid|escape:url}">
			<i class="icon-list-alt {if isset($board) && $board_item.boardid == $board.boardid}icon-white{/if}" {if count($board_item.childs) > 0}onClick="$('#board-childnav{$board_item.boardid}').fadeToggle(200);return false;"{/if}></i>
